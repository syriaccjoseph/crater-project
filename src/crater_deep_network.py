...
def shallow(n=3, epochs=60):
    ...
    for j in range(n):
        ...
        net.SGD(
            training_data, epochs, mini_batch_size, 3,
            validation_data, test_data)
        nets.append(net)
    return nets

def shallow1(n=3, epochs=60):
    nets = []
    for j in range(n):
        print "A shallow net with 100 hidden neurons"
        net = Network([
...
def basic_conv(n=3, epochs=60):
    for j in range(n):
        ...
        net = Network([
            ...
            FullyConnectedLayer(n_in=5 * 98 * 98, n_out=40, activation_fn=ReLU),
            SoftmaxLayer(n_in=40, n_out=2)], mini_batch_size)
        net.SGD(
            training_data, epochs, mini_batch_size, 0.1, validation_data, test_data, lmbda=.5)
    return net 

def basic_conv1(n=3, epochs=60):
    for j in range(n):
        print "Conv + FC architecture"
        net = Network([
    ...
    for j in range(3):
        ...
        net = Network([
            ...
            ConvPoolLayer(image_shape=(mini_batch_size, 20, 98, 98),
                          ...
                          activation_fn=activation_fn),
            FullyConnectedLayer(
                n_in=40*47*47, n_out=100, activation_fn=activation_fn),
            SoftmaxLayer(n_in=100, n_out=2)], mini_batch_size)
        net.SGD(training_data, 60, mini_batch_size, 0.1, validation_data, test_data)
    return net

def dbl_conv1(activation_fn=sigmoid):
    for j in range(3):
        print "Conv + Conv + FC architecture"
        net = Network([
            ConvPoolLayer(image_shape=(mini_batch_size, 1, 28, 28), 
        ...
        for j in range(3):
            ...
            net = Network([
                ...
                ConvPoolLayer(image_shape=(mini_batch_size, 20, 98, 98),
                              filter_shape=(40, 20, 5, 5),
                              poolsize=(2, 2)),
                FullyConnectedLayer(n_in=40*47*47, n_out=100),
                SoftmaxLayer(n_in=100, n_out=2)], mini_batch_size)
            net.SGD(training_data, 60, mini_batch_size, 0.01, validation_data, test_data, lmbda=lmbda)

def regularized_dbl_conv1():
    for lmbda in [1.0, 10.0, 100.0]:
        for j in range(3):
            print "Conv + Conv + FC num %s, with regularization %s" % (j, lmbda)
            net = Network([
                ConvPoolLayer(image_shape=(mini_batch_size, 1, 28, 28), 
...
def dbl_conv_relu():
    for lmbda in [1.0, 10.0, 100.0]:
        for j in range(3):
            ...
            net = Network([
                ConvPoolLayer(image_shape=(mini_batch_size, 1, 200, 200),
                              filter_shape=(70, 1, 5, 5),
                              poolsize=(2, 2),
                              activation_fn=ReLU),
                ConvPoolLayer(image_shape=(mini_batch_size, 70, 98, 98),
                              filter_shape=(120, 70, 5, 5),
                              poolsize=(2, 2),
                              activation_fn=ReLU),
                FullyConnectedLayer(n_in=120*47*47, n_out=100, activation_fn=ReLU),
                SoftmaxLayer(n_in=100, n_out=2)], mini_batch_size)
            net.SGD(training_data, 60, mini_batch_size, 1.2, validation_data, test_data, lmbda=lmbda)
